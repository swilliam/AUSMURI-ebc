...
import cv2
from circlemaze import F,B,N,H,maze, MAZE_ID
...
def near_wall_dir(ix, iy, dx, dy):
    ...
    for x,y in path_to_wall:
        if maze[x,y] !=F:
            if maze[x+1,y] ==F:
                wdir = math.atan2(y,-x)*180/np.pi   #west
            elif maze[x-1,y] ==F:
                wdir = math.atan2(y,-x)*180/np.pi   #west90  #east
            elif maze[x,y-1] ==F:
                wdir = math.atan2(y,-x)*180/np.pi   #west0  #north
            elif maze[x,y+1] ==F:
                wdir = math.atan2(y,-x)*180/np.pi   #west180     #south
            else:
                raise 'no wall'
            iswall = True
            break
